...
export const ProductCard = ({id, name, price, rating, image}:Props) => {
  ...
  //Agrega un producto en la cookie cada que el usuario da click en el boton agregar
  const onAddToCart = ()=>{
    addProductToCart(id)
    router.refresh()              //Refresca la pagina cuando el usuario agrega un item al carrito de compras
  }

  //Elimina productos del carrito de compras
  const onRemoveFromCart = () => {
    removeProductFromCart(id)         //Elimina el producto de las cookies
    router.refresh()  
  }
